...
Функция ЗаполненыНеобходимыеПоляОбъекта(Форма,ПроверяемыеПоля,СтандартнаяОбработка=Истина,Отказ=Ложь,СообщатьОбОшибках=Истина) Экспорт
	...
	Для Каждого ПроверяемоеПоле Из ПроверяемыеПоля Цикл
		Если НЕ ЗначениеЗаполнено(Форма.Объект[ПроверяемоеПоле]) Тогда МассивОшибок.Добавить(уидОбработкаСтрокКлиентСервер.ИмяВСиноним(ПроверяемоеПоле)); КонецЕсли;
	КонецЦикла;
...
Функция ЗаполненыНеобходимыеПоляФормы(Форма,ПроверяемыеПоля,СтандартнаяОбработка=Истина,Отказ=Ложь,СообщатьОбОшибках=Истина) Экспорт
	...
	МассивОшибок = Новый Массив;
	Для Каждого ПроверяемоеПоле Из ПроверяемыеПоля Цикл
		Если НЕ ЗначениеЗаполнено(Форма[ПроверяемоеПоле]) Тогда МассивОшибок.Добавить(уидОбработкаСтрокКлиентСервер.ИмяВСиноним(ПроверяемоеПоле)); КонецЕсли;
	КонецЦикла;
	Если МассивОшибок.Количество() > 0 Тогда
		Если СообщатьОбОшибках Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru='Для продолжения заполните %1: %2'"), ?(МассивОшибок.Количество() > 1, "поля", "поле"), СтрСоединить(МассивОшибок, ", "));
...
Функция ЗаполненыНеобходимыеПоляЗаписиРегистра(Форма,ПроверяемыеПоля,СтандартнаяОбработка=Истина,Отказ=Ложь,СообщатьОбОшибках=Истина) Экспорт
	...
	МассивОшибок = Новый Массив;
	Для Каждого ПроверяемоеПоле Из ПроверяемыеПоля Цикл
		Если НЕ ЗначениеЗаполнено(Форма.Запись[ПроверяемоеПоле]) Тогда МассивОшибок.Добавить(уидОбработкаСтрокКлиентСервер.ИмяВСиноним(ПроверяемоеПоле)); КонецЕсли;
	КонецЦикла;
	Если МассивОшибок.Количество() > 0 Тогда
		Если СообщатьОбОшибках Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru='Для продолжения заполните %1: %2'"), ?(МассивОшибок.Количество() > 1, "поля", "поле"), СтрСоединить(МассивОшибок, ", "));
...
Функция ЗаполненыНеобходимыеПоляСтрокиТаблицыФормы(Форма,ИмяТаблицыФормы,ПроверяемыеПоля,СтандартнаяОбработка=Истина,Отказ=Ложь,СообщатьОбОшибках=Истина) Экспорт
	...
	МассивОшибок = Новый Массив;
	Для Каждого ПроверяемоеПоле Из ПроверяемыеПоля Цикл
		Если Не ЗначениеЗаполнено(Форма.Элементы[ИмяТаблицыФормы].ТекущиеДанные[ПроверяемоеПоле]) Тогда МассивОшибок.Добавить(уидОбработкаСтрокКлиентСервер.ИмяВСиноним(ПроверяемоеПоле)); КонецЕсли;
	КонецЦикла;
	Если МассивОшибок.Количество() > 0 Тогда
		Если СообщатьОбОшибках Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru='Для продолжения заполните %1 таблицы %2: %3'"), ?(МассивОшибок.Количество() > 1, "поля", "поле"), ?(ПустаяСтрока(Форма.Элементы[ИмяТаблицыФормы].Заголовок), уидОбработкаСтрокКлиентСервер.ИмяВСиноним(Форма.Элементы[ИмяТаблицыФормы].Имя), Форма.Элементы[ИмяТаблицыФормы].Заголовок), СтрСоединить(МассивОшибок, ", "));
			ПоказатьПредупреждение(, ТекстСообщения, 60, НСтр("ru='ПРЕДУПРЕЖДЕНИЕ';"));
		КонецЕсли;
		СтандартнаяОбработка = Не СтандартнаяОбработка;
		Отказ = Не Отказ;
...
Функция ЗаполненыНеобходимыеКолонкиТабличнойЧасти(Форма,ИмяТабличнойЧасти,ПроверяемыеКолонки,СтандартнаяОбработка=Истина,Отказ=Ложь,СообщатьОбОшибках=Истина) Экспорт
	...
	Если Форма.Объект[ИмяТабличнойЧасти].Количество()=0 Тогда
		Для Каждого ПроверяемаяКолонка Из ПроверяемыеКолонки Цикл
			МассивОшибок.Добавить(уидОбработкаСтрокКлиентСервер.ИмяВСиноним(ПроверяемаяКолонка));
		КонецЦикла;
	...
	Если МассивОшибок.Количество() > 0 Тогда
		Если СообщатьОбОшибках Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru='Для продолжения заполните %1 таблицы %2: %3'"), ?(МассивОшибок.Количество() > 1, "колонки", "колонку"), ?(ПустаяСтрока(Форма.Элементы[ИмяТабличнойЧасти].Заголовок), уидОбработкаСтрокКлиентСервер.ИмяВСиноним(Форма.Элементы[ИмяТабличнойЧасти].Имя), Форма.Элементы[ИмяТабличнойЧасти].Заголовок), СтрСоединить(МассивОшибок, ", "));
			ПоказатьПредупреждение(, ТекстСообщения, 60, НСтр("ru='ПРЕДУПРЕЖДЕНИЕ';"));
		КонецЕсли;
		СтандартнаяОбработка = Не СтандартнаяОбработка;
		Отказ = Не Отказ;
...
Функция ПустыеНеобходимыеПоляОбъекта(Форма,ПроверяемыеПоля,СтандартнаяОбработка=Истина,Отказ=Ложь,СообщатьОбОшибках=Истина) Экспорт
	...
	МассивОшибок = Новый Массив;
	Для Каждого ПроверяемоеПоле Из ПроверяемыеПоля Цикл
		Если ЗначениеЗаполнено(Форма.Объект[ПроверяемоеПоле]) Тогда МассивОшибок.Добавить(уидОбработкаСтрокКлиентСервер.ИмяВСиноним(ПроверяемоеПоле)); КонецЕсли;
	КонецЦикла;
	Если МассивОшибок.Количество() > 0 Тогда
		Если СообщатьОбОшибках Тогда
